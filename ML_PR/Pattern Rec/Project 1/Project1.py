...
def BatchPerceptron(X:np.ndarray, epochs=1000, seed=0):
    """
    @purpose:
...
for label in labels:
    ...
    row = np.zeros(unique_labels)
    # Set the value at the corresponding column to 1
    row[label - 1] = 1
    # Append the row to the target matrix
    T_matrix.append(row)
...
xline = np.linspace(F34_X[:, 1].min(), F34_X[:, 1].max())
y1_line = -(W_Matrix[0, 0] + W_Matrix[0, 1] * xline) / W_Matrix[0, 2]
y2_line = -(W_Matrix[1, 0] + W_Matrix[1, 1] * xline) / W_Matrix[1, 2]
y3_line = -(W_Matrix[2, 0] + W_Matrix[2, 1] * xline) / W_Matrix[2, 2]

plt.scatter(virgin[:, 3], virgin[:, 4], marker='x', color='red')
plt.scatter(setosa[:, 3], setosa[:, 4], marker='^', color='black')
plt.scatter(versi[:, 3], versi[:, 4], marker='o', color='blue')
plt.plot(xline, y1_line, color='green', label='W[0]')
plt.plot(xline, y2_line, color='purple', label='W[1]')
plt.plot(xline, y3_line, color='orange', label='W[2]')
plt.xlabel('Feature 3')
plt.ylabel('Feature 4')
plt.xlim(-2,2)
plt.ylim(-2,2)
